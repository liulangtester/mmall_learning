...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //填充orderItem返回OrderItemList 订单号还未填充
        ServerResponse serverResponse = this.getCartOrderItem(userId, cartList);
        //如果不是在售状态以及库存充足，直接返回错误信息
        if (!serverResponse.isSucess()) {
            return serverResponse;
        }
        ...
        //把orderItem的订单号的填充上  订单中的所有产品都是同一个订单号
        for (OrderItem orderItem : orderItemList) {
    ...
    private OrderVo assembleOrderVo(Order order, List<OrderItem> orderItemList) {
        ...
        //根据传入的支付方式(如在线支付的code为1)code获取到对应的枚举，再获取该枚举的描述value
    ...
    /**
     * 根据从购物车中获取的已勾选的商品，进行售卖状态、库存校验、计算单个商品总价等
     * @param userId
     * @param cartList
     * @return
     */
...
//        goodsDetailList.add(goods1);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                //path最后没有/, /qr-%s.png一定要加/   以订单号命名，%s会被后面的订单号替换
                //二维码路径
                ...
                //上传二维码到FTP  也可以直接放路径qrpath，但是后面要获取文件名，还要再拆分，这样不需要拆分，直接能获取到文件名
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order == null) {
            return ServerResponse.createByErrorMessage("happymmall商城订单，回调忽略");
        }
        if (order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()) {
            //状态码大于已付款的状态码就不需要回调了，要返回success才不会重复回调
            return ServerResponse.createBySuccess("支付宝重复调用");
        }

        //回调回来的参数tradeStatus，如果是已付款状态就把order里的状态进行持久化
